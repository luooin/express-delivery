...
import com.example.express.domain.vo.*;
...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
    ...
    @Autowired
    private OrderEvaluateMapper orderEventMapper;
    ...
    @Override
    public ResponseResult batchAdd(BatchAddVO vo) {
    ...
    @Override
    public ResponseResult batchUpdate(BatchUpdateVO vo) {
        OrderInfo orderInfo = orderInfoMapper.selectById(vo.getId());
        if(orderInfo != null) {
            BeanUtils.copyProperties(vo, orderInfo);
            orderInfo.setUpdateDate(LocalDateTime.now());
    ...
    public ResponseResult delete(String id) {
        ...
        if(orderInfo != null) {
            orderInfo.setHasDelete(1);
            orderInfo.setDeleteType(OrderDeleteEnum.MANUAL);
            orderInfo.setUpdateDate(LocalDateTime.now());
            orderInfoMapper.updateById(orderInfo);
            return ResponseResult.success();
        }
    ...
    public void pickupMethodx(String pickupMethodx,String id) {
        List<OrderInfo> pickupMethod = orderInfoMapper.selectList(new QueryWrapper<OrderInfo>().eq("pickup_method", pickupMethodx));

        pickupMethod.forEach(orderInfo ->{
            OrderEvaluate orderEvaluate = new OrderEvaluate();
            orderEvaluate.setId(orderInfo.getId());
            orderEvaluate.setUserId(orderInfo.getUserId());
            orderEvaluate.setCourierId(id);
            orderEvaluate.setUpdateDate(LocalDateTime.now());
            orderEventMapper.insert(orderEvaluate);
    ...
    @Override
    public BootstrapTableVO<OrderInfoVo> getDashboardData(String startDate, String endDate, Page<OrderInfo> page, String kuidi) {

        BootstrapTableVO<OrderInfoVo> vo = new BootstrapTableVO<>();
        if(StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)) {
            ...
            IPage<OrderInfo> orderInfoIPage = orderInfoMapper.selectPage(page, new QueryWrapper<OrderInfo>().eq("courier_id", kuidi).between("create_date", start, end));
            ...
            vo.setTotal((long) orderInfoVos.size());
            vo.setRows(orderInfoVos);
        }
        return vo;
    }

    @Override
    public ResponseResult placeOrder(OrderInfo orderInfo) {
        ...
        orderInfo.setId(String.valueOf(l));
        orderInfo.setUserId(user.getId());
        orderInfo.setOrderStatus(OrderStatusEnum.WAIT_DIST);
        orderInfoMapper.insert(orderInfo);


        orderPaymentMapper.insert(OrderPayment.builder()
                .orderId(String.valueOf(l))
