...
public class OrderApiController {
    ...
    public BootstrapTableVO listSelfOrder(@RequestParam(required = false, defaultValue = "1") Integer current,
        ...
        if(StringUtils.isNotBlank(id)) {
            sql.append(" AND info.odd = ").append("'"+id+"'");
        }
    ...
    public BootstrapTableVO<CourierOrderVO> listWaitDistOrder(@RequestParam(required = false, defaultValue = "1") Integer current,
        ...
        sql.append(" AND info.has_delete = 0");
    ...
    public ResponseResult batchAdd(@RequestBody BatchAddVO vo) {
        return orderInfoService.batchAdd(vo);
    }

    @PutMapping("/batch-Update")
    public ResponseResult batchUpdate(@RequestBody BatchUpdateVO vo) {
        return orderInfoService.batchUpdate(vo);
    }
    ...
    @PostMapping("/pickupMethodx")
    public ResponseResult pickupMethodx(String pickupMethodx,String id) {
       orderInfoService.pickupMethodx(pickupMethodx,id);
        return ResponseResult.success();
    }

    @GetMapping("/get-dashboard-data")
    public BootstrapTableVO<OrderInfoVo> getDashboardData(@RequestParam(required = false, defaultValue = "1") Integer current,
                                                          @RequestParam(required = false, defaultValue = "10") Integer size,
                                                          String startDate, String endDate, String kuidi) {

        Page<OrderInfo> page = new Page<>(current, size);
        return orderInfoService.getDashboardData(startDate,endDate, page,kuidi);
    }
     ...
     */
    @PostMapping("/place")
    public ResponseResult placeOrder(@RequestBody OrderInfo orderInfo) {
        return orderInfoService.placeOrder(orderInfo);
    }
